...
public class MyApp {
    ...
    public boolean cekMaster(String tabelMaster){
    ...
    public boolean cekTrans(String tabelTrans){
        String sql = "select * from "+tabelTrans ;
        if (utilitas.getSession(Config.getIDFullVersion(),false)){
            return false ;
        } else if (database.getCount(sql) > Config.getJumTrans()){
            return true ;
        } else {
            return false ;
        }
