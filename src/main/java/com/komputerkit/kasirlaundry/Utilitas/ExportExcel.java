...
public class ExportExcel {
    ...
    public void create() throws IOException, WriteException {
        ...
        if (db.getCount(query) > 0){
            ...
            int no = 0 ;
            while(c.moveToNext()){
                addCell(utilitas.intToStr(no+1),no,row);
                no++ ;
                for(int i = 0 ; i < kolom.length ; i++){
                    addCell(db.getString(c,kolom[i]),no++,row);
                }
                row++ ;
            }
    ...
    public void create2() throws IOException, WriteException {
        ...
        if (db.getCount(query) > 0 && kolom.length == kolom2.length){
            ...
            while(c.moveToNext()){
                int col = 0 ;
                addCell(utilitas.intToStr(no++),col++,row);
                for(int i = 0 ; i < kolom2.length ; i++){
                    String[] temp = kolom2[i].split("__") ;
                    if (temp.length == 2){
                        if (temp[1].equals("float")){
                            addCell(utilitas.removeE(db.getString(c,temp[0])),col++,row);
                        }
                    } else {
                        addCell(db.getString(c,kolom2[i]),col++,row);
                    }
                }
                row++ ;
            }
